...
void makeTrees(std::string l1_collection ,std::string outDir, TFile *fout, TFile *fin){
	...
	if (fptQCD){
	   ...
	   std::cout << " pT reweight not found, don't worry, we don't need it unless for training " << std::endl;
	}
	...
	std::vector<float> *jet_area_strip1_chunky= 0;
	std::vector<float> *jet_area_strip2_chunky= 0;
	std::vector<float> *jet_energy_strip1_chunky= 0;
	std::vector<float> *jet_energy_strip2_chunky= 0;
	...
	l1_tree->SetBranchAddress("jetOuterStripsArea_1_5400_chunky",&jet_area_strip1_chunky);
	...
	// Now BEGIN LOOP!
	if (MAXEVENTS>0) std::cout << "fraction of events = " << fraction << std::endl;
	...
	for (unsigned int ev_i=0;ev_i<l1_tree->GetEntries();ev_i++){
	  ...
	  for (unsigned int i=0;i<pt_l1->size();i++){
		...
		tmPT  = (*pt_l1)[i];
	        ...
	        // apply seed threshold in PHYSICAL UNITS 
		if (0.5*( (*es_l1)[i] + (*hs_l1)[i] ) < minseed) continue;

		if (SUBDONUT>0) {  // leave ENERGY in unphysical (L1) scale before Donut subtraction

		  if (SUBDONUT==1) {  // Think donut 
			...
			tmPT -= dnutE; 
		  } else if (SUBDONUT==2){ // chunky donut
			double stripE = (*jet_energy_strip1_chunky)[i] + (*jet_energy_strip2_chunky)[i] ;
			double dnutE = stripE*( (*jet_area)[i]/( (*jet_area_strip1_chunky)[i] + (*jet_area_strip2_chunky)[i] ) );
			//std::cout << "pt, eta, dnut " << tmPT <<", "<< tmETA << ", " << dnutE << std::endl;
			tmPT -= dnutE; 
		  } else if (SUBDONUT==3){ // jet-based median rho
			tmPT -= rho*(*jet_area)[i];
		  }
		}
